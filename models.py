...
class NearEarthObject:
    ...
    def __str__(self):
        ...
        if math.isnan(self.diameter):
            return f"NEO {self.fullname} is {'' if self.hazardous else 'not'} potentially hazardous."
        return f"NEO {self.fullname} has a diameter of {format(self.diameter), '.3f'} km " \
...
class CloseApproach:
    ...
    def time_str(self):
        ...
        """
        return datetime_to_str(self.time) if self.time else "Not a readable time."

    def __str__(self):
        """Return `str(self)`."""
        # The project instructions include one possibility. Peek at the __repr__
        # method for examples of advanced string formatting.
        return f"At {self.time_str}, '{self.neo.fullname}' approaches Earth at a distance of {self.distance:.2f} au " \
